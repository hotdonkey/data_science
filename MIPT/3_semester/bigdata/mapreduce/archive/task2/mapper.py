...
def timecounter(business_id, hours):
    # проверяем (для использования компаратора и составного ключа приклеил ко всем 1- не знаю насколько это костыльно)
    if not hours:
        return f"{business_id}\t0\t1"
    ...
    return f"{business_id}\t{total_minutes}\t1"
...
for line in input_stream:#sys.stdin:
    ...
    try:
        ...
        minutes = timecounter(business_id, hours)
        #print(minutes)
        print(minutes, file=output_stream)
